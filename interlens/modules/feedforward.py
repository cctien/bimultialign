...
class FeedForward(torch.nn.Module, FromParams):
    ...
    The class if bought from [AllenNLP](https://github.com/allenai/allennlp/blob/master/allennlp/modules/feedforward.py), 
    with a single revision: when the activation function is SELU, AlphaDropout is used instead of Dropout.
    ...
    def __init__(
        ...
        if len(hidden_dims) != num_layers:
            raise ConfigurationError(f"len(hidden_dims) {len(hidden_dims)} "
                                     f"!= num_layers {num_layers}")
        if len(activations) != num_layers:
            raise ConfigurationError(f"len(activations) {len(activations)} "
                                     f"!= num_layers {num_layers}")
        if len(dropout) != num_layers:
            raise ConfigurationError(f"len(dropout) {len(dropout)} "
                                     f"!= num_layers {num_layers}")
        ...
        dropout_layers = []
        for i, dropout_value in enumerate(dropout):
            if activations[i]._get_name() == 'SELU':
                logger.info("Use AlphaDropout as the activation is SELU.")
                dropout_layers.append(torch.nn.AlphaDropout(p=dropout_value))
            else:
                dropout_layers.append(torch.nn.Dropout(p=dropout_value))
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        output = inputs
        for layer, activation, dropout in zip(self._linear_layers,
                                              self._activations,
                                              self._dropout):
